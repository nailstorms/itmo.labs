...
public class EquationSystemSolverTest {
    ...
    public Object[][] equationSystemTestData() {
        return new Object[][]
                {
                        {-1.570796, -1.0, 0.0, Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY, Double.NaN, Double.NaN, Double.NaN, Double.NaN, Double.NaN},
                        ...
                        {0.499263, 0.47877863, 0.87793566, 0.54534591, 1.13903563, -0.69462226, -1.0021281, -0.63227243, -0.30167061, 1.52105},
                        ...
                        {20.5635, 0.35124526, 0.93628348, 0.37514841, 1.06805259, 3.02351766, 4.36201393, 2.75212438, 1.31309704, -248.58624}
                };
    }
    ...
    public void testEquationSystemSolverStubbed(Double x, Double sin, Double cos, Double tan, Double sec,
        ...
        Assert.assertEquals(DoubleUtil.round(solver.solve(x)), expected);
    }
    ...
    private void mockTrig(Double x, Double sin, Double cos, Double tan, Double sec) {
        when(mockITrig.sin(eq(x))).thenReturn(sin);
        when(mockITrig.cos(eq(x))).thenReturn(cos);
        when(mockITrig.tan(eq(x))).thenReturn(tan);
        when(mockITrig.sec(eq(x))).thenReturn(sec);
    }

    private void mockLogarithm(Double x, Double ln, Double log2, Double log3, Double log10) {
        when(mockILogarithm.ln(eq(x))).thenReturn(ln);
        when(mockILogarithm.log(eq(x), eq(2.0))).thenReturn(log2);
        when(mockILogarithm.log(eq(x), eq(3.0))).thenReturn(log3);
        when(mockILogarithm.log(eq(x), eq(10.0))).thenReturn(log10);
    }
}
